...
function buildFreeTierSummary(evidenceById, subscores, overallScoreInfo) {
  ...
  if (robots) {
    const oaiBlocked =
      robots.rawEvidence.botRules &&
      robots.rawEvidence.botRules["oai-searchbot"]
...
function buildNarrativePrompt(
  ...
  const evidenceForPrompt = Object.fromEntries(
    Object.entries(evidenceById).map(([id, ev]) => [
      id,
      { checkId: ev.checkId, rawEvidence: ev.rawEvidence },
    ]),
  );
...
async function callNarrativeModel(messages) {
  ...
  if (!apiKey) {
    throw new Error(
      "OPENAI_API_KEY is not configured — cannot generate paid narrative.",
    );
  }
...
function validateNarrative(narrativeText, evidenceById) {
  ...
  for (const id of citedIds) {
    if (!evidenceById[id]) {
      problems.push(
        `Citation [[${id}]] does not match any evidence checkId gathered for this assessment.`,
      );
    }
  }
  ...
  while ((numMatch = numberRe.exec(narrativeText)) !== null) {
    ...
    if (!CITATION_RE.test(window)) {
      problems.push(
        `Number "${numMatch[0]}" near position ${numMatch.index} has no nearby citation.`,
      );
    }
    CITATION_RE.lastIndex = 0;
  }
...
async function generateValidatedNarrative(
  ...
  const messages = buildNarrativePrompt(
    evidenceById,
    subscores,
    overallScore,
    businessName,
