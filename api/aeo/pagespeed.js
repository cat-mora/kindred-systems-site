...
async function fetchPageSpeed(url, apiKey, strategy = 'mobile') {
  ...
  if (!res.ok) {
    ...
    throw new Error(`PageSpeed Insights returned HTTP ${res.status}: ${body.slice(0, 300)}`);
  }
  return res.json();
}
...
function parsePageSpeedResponse(data) {
  ...
  const cwv = data.loadingExperience && data.loadingExperience.metrics ? data.loadingExperience.metrics : {};
  ...
  return {
    ...
    coreWebVitals: {
      ...
      interactionToNextPaintMs: audits['interaction-to-next-paint']
        ...
        : null,
      totalBlockingTimeMs: audits['total-blocking-time'] ? audits['total-blocking-time'].numericValue : null,
    },
    fieldData: {
      lcpCategory: cwv.LARGEST_CONTENTFUL_PAINT_MS ? cwv.LARGEST_CONTENTFUL_PAINT_MS.category : null,
      clsCategory: cwv.CUMULATIVE_LAYOUT_SHIFT_SCORE ? cwv.CUMULATIVE_LAYOUT_SHIFT_SCORE.category : null,
    },
  };
}
